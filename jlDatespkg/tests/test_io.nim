...
test "Test string/show representation of initDate":
    ...
    check $(initDateTime(2000, 1, 1, 0, 0, 0, 500)) == "2000-01-01T00:00:00.5"
...
test "Common Parsing Patterns":
    ...
    let f = "%y-%m-%d"
    let a = "96-01-15"
    check strptime(a, f) + Year(1900) == dt
    check strftime(dt, f) == a
    let a1 = "96-1-15"
    check strptime(a1, f) + Year(1900) == dt
    check strftime(dt, f) == "96-01-15"
    let a2 = "96-1-1"
    check strptime(a2, f) + Year(1900) + Day(14) == dt
    check strftime(dt - Day(14), "%y-%m-%d") == "96-01-01"
    let a3 = "1996-1-15"
    check strptime(a3, f) == dt
...
test "monthname":
    ...
    let b = "96/Feb/15"
    let dt = initDateTime(1996, 1, 15)
    check strptime(b, f) + Year(1900) == dt + Month(1)
    check strftime(dt + Month(1), f) == b
    let b1 = "1996/Feb/15"
    check strptime(b1, f) == dt + Month(1)
    check strftime(dt + Month(1), "%Y/%b/%d") == b1
    let b2 = "96/Feb/1"
    check strptime(b2, f) + Year(1900) + Day(14) == dt + Month(1)
    ...
    expect ValueError:
        echo strptime(b3, f)
    try:
        echo strptime("2012/2/20T9:9:31.25i90", "%y/%m/%dT%M:%M:%S.%f")
        check true
    except:
...
test "colon separator":
    ...
    let c = "96:15:01"
    let dt = initDateTime(1996, 1, 15)
    check strptime(c, f) + Year(1900) == dt
    check strftime(dt, f) == c
    let c1 = "1996:15:01"
    check strptime(c1, f) == dt
    check strftime(dt, "%Y:%d:%m") == c1
    let c2 = "96:15:1"
    check strptime(c2, f) + Year(1900) == dt
    check strftime(dt, "%y:%d:%m") == "96:15:01"
    let c3 = "96:1:01"
    check strptime(c3, f) + Year(1900) + Day(14) == dt
    check strftime(dt - Day(14), "%y:%m:%d") == "96:01:01"
    let c4 = "1996:15:01 # random comment"
    check strptime(c4, f) == dt

test "comma separator":
    let f = "%Y, %b, %d"
    let d = "1996, Jan, 15"
    let dt = initDateTime(1996, 1, 15)
    check strptime(d, f) == dt
    check strftime(dt, f) == d
    let d1 = "96, Jan, 15"
    check strptime(d1, f) + Year(1900) == dt
    check strftime(dt, "%y, %b, %d") == d1
    let d2 = "1996, Jan, 1"
    check strptime(d2, f) + Day(14) == dt
...
test "point separator":
    ...
    let dt = initDateTime(1996, 1, 15)
    check strptime(e, f) == dt
    check strftime(dt, f) == e
    let e1 = "96.January.15"
    check strptime(e1, f) + Year(1900) == dt
...
test "space separator":
    ...
    let f = "1996 1 15"
    let dt = initDateTime(1996, 1, 15)
    check strptime(f, fo) == dt
    check strftime(dt, fo) == "1996 01 15"
    let f1 = "1996 01 15"
    check strptime(f1, fo) == dt
    check strftime(dt, "%Y %m %d") == f1
    let f2 = "1996 1 1"
    check strptime(f2, fo) + Day(14) == dt
...
test "dash separator":
    ...
    var f = "%Y-%m-%d zzz"
    let dt = initDateTime(1996, 1, 15)
    check strptime(j, f) == dt
    check strftime(dt, f) == j & " zzz"
    let k = "1996-01-15 10:00:00"
    f = "%Y-%m-%d %H:%M:%S zzz"
    check strptime(k, f) == dt + Hour(10)
    check strftime(dt + Hour(10), f) == k & " zzz"
    let l = "1996-01-15 10:10:10.25"
    f = "%Y-%m-%d %H:%M:%S.%f zzz"
    check strptime(l, f) == dt + Hour(10) + Minute(10) + Second(10) + Millisecond(250)
...
test "slash separator":
    ...
    var f = "%m/%d/%Y"
    let dt = initDateTime(1996, 1, 15)
    check strptime(r, f) == dt
    check strftime(dt, f) == "01/15/1996"
    let s = "19960115"
    f = "%Y%m%d"
    check strptime(s, f) == dt
    check strftime(dt, f) == s
    let v = "1996-01-15 10:00:00"
    f = "%Y-%m-%d %H:%M:%S"
    check strptime(v, f) == dt + Hour(10)
    check strftime(dt + Hour(10), f) == v
    let w = "1996-01-15T10:00:00"
    f = "%Y-%m-%dT%H:%M:%S zzz"
    check strptime(w, f) == dt + Hour(10)
    check strftime(dt + Hour(10), f) == w & " zzz"

    f = "%Y/%m"
    let y = "1996/1"
    check strptime(y, f) == dt - Day(14)
    check strftime(dt, f) == "1996/01"
    let y1 = "1996/1/15"
    check strptime(y1, f) == dt - Day(14)
    # check_throws ArgumentError initDateTime(y1, f)
    let y2 = "96/1"
    check strptime(y2, f) + Year(1900) == dt - Day(14)
    check strftime(dt, "%y/%m") == "96/01"

    f = "%Y"
    let z = "1996"
    check strptime(z, f) == dt - Day(14)
    check strftime(dt, f) == z
    let z1 = "1996-3"
    check strptime(z1, f) == dt - Day(14)
    check strftime(dt, f) == z
    let z2 = "1996-3-1"
    check strptime(z2, f) == dt - Day(14)
    check strftime(dt, f) == z

    let aa = "1/5/1996"
    f = "%m/%d/%Y"
    check strptime(aa, f) == dt - Day(10)
    check strftime(dt - Day(10), f) == "01/05/1996"
    let bb = "5/1/1996"
    f = "%d/%m/%Y"
    check strptime(bb, f) == dt - Day(10)
    check strftime(dt - Day(10), f) == "05/01/1996"
    let cc = "01151996"
    f = "%m%d%Y"
    check strptime(cc, f) == dt
    check strftime(dt, f) == cc
    let dd = "15011996"
    f = "%d%m%Y"
    check strptime(dd, f) == dt
    check strftime(dt, f) == dd
    let ee = "01199615"
    f = "%m%Y%d"
    check strptime(ee, f) == dt
    check strftime(dt, f) == ee
    let ff = "1996-15-Jan"
    f = "%Y-%d-%b"
    check strptime(ff, f) == dt
    check strftime(dt, f) == ff
    let gg = "Jan-1996-15"
    f = "%b-%Y-%d"
    check strptime(gg, f) == dt
    check strftime(dt, f) == gg
    let hh = "1996#1#15"
    f = "%Y#%m#%d"
    check strptime(hh, f) == dt
...
test "French: from Milan":
    let f = "%d/%m/%Y"
    let f2 = "%d/%m/%y"
    check strptime("28/05/2014", f) == initDateTime(2014, 5, 28)
    check strptime("28/05/14", f2) + Year(2000) == initDateTime(2014, 5, 28)

test "Customizing locale":
    jlDates.LOCALES["french"] = initDateLocale(
    ...
    var f = "%d %b %Y"
    check strptime("28 mai 2014", f, locale="french") == initDateTime(2014, 5, 28)
    check strftime(initDateTime(2014, 5, 28), f, locale="french") == "28 mai 2014"
    check strptime("28 févr 2014", f, locale="french") == initDateTime(2014, 2, 28)
    check strftime(initDateTime(2014, 2, 28), f, locale="french") == "28 févr 2014"
    check strptime("28 août 2014", f, locale="french") == initDateTime(2014, 8, 28)
    check strftime(initDateTime(2014, 8, 28), f, locale="french") == "28 août 2014"
    check strptime("28 avril 2014", f, locale="french") == initDateTime(2014, 4, 28)
...
#    check strptime("292277025-08-17T07:12:55.295", f) == initDateTime(292_277_025,8,17,7,12,55,296)
    ...
    check strptime("20140529 123041", "%Y%m%d %H%M%S") == initDateTime(2014, 5, 29, 12, 30, 41)


test "julialang Issue 13":
    let t = initDateTime(1, 1, 1, 14, 51, 0, 118)
    check strptime("[14:51:00.118]", "[%H:%M:%S.%f]") == t
    check strptime("14:51:00.118", "%H:%M:%S.%f") == t
    check strptime("[14:51:00.118?", "[%H:%M:%S.%f?") == t
    check strptime("?14:51:00.118?", "?%H:%M:%S.%f?") == t
    check strptime("x14:51:00.118", "x%H:%M:%S.%f") == t
    check strptime("14:51:00.118]", "%H:%M:%S.%f]") == t

test "RFC1123Format":
    let dt = initDateTime(2014, 8, 23, 17, 22, 15)
    check strftime(dt, "$rfc1123") == "Sat, 23 Aug 2014 17:22:15 GMT"
    check strptime(strftime(dt, "$rfc1123"), "$rfc1123") == dt
    ...
    check strptime("Sat, 12 Nov 2016 07:45:36", "$rfc1123") == dt1
    check strptime("Mon, 12 Nov 2016 07:45:36", "$rfc1123") == dt1  # Wrong day of week
    expect ValueError:
        echo strptime("Foo, 12 Nov 2016 07:45:36", "$rfc1123")
    # check_throws ArgumentError parse(initDate, "Foo, 12 Nov 2016 07:45:36", RFC1123Format)


test "julialang Issue 15195":
    let f = "%Y"
    check strftime(strptime("1999", f), f) == "1999"
